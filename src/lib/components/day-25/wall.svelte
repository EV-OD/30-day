<script lang="ts">
    ...
    export let textureMeshWidth = 0.01;
    ...
    $: textureWidth = ((fakeWidth - offset) / textureCount) - (textureGap * (textureCount - 1) / textureCount);
...
<T.Group  position={[0,0,0]}>
    ...
    <!-- wall texture  -->
    <T.Group position={[offset / (2 * (isLeftOffset ? 1 : -1)) - overallTextureOffset,0,offset / 2]}>
        ...
        <T.Mesh position={[(offset - fakeWidth / 2) + i * (textureWidth + textureGap),0,textureMeshWidth / 2]}>
